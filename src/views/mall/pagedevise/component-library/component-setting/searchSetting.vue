        ...
        <el-form ref="form" label-width="90px" :model="formData">
          <div>
            ...
            <el-form-item label="右侧图片">
              <el-radio-group v-model="formData.rightImageShow">
                <el-radio label="0">关闭</el-radio>
                <el-radio label="1">开启</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="图片尺寸" v-show="formData.rightImageShow==1">
              <div class="text-black">建议宽高为30px*25px</div>
            </el-form-item>
            <el-form-item label="图片" v-show="formData.rightImageShow==1">
              <MaterialList
                :value="formData.rightImageUrl?[formData.rightImageUrl]:[]"
                @sureSuccess="formData.rightImageUrl = $event?$event[0]:''"
                @deleteMaterial="formData.rightImageUrl = ''"
                type="image" shopId="-1" :num=1 :width="80" :height="48"></MaterialList>
            </el-form-item>
            <el-form-item label="图片宽度" v-show="formData.rightImageShow==1">
              <el-input v-model="formData.rightImageWidth" size="mini"  type="number" placeholder="宽度">
                <template slot="append">px</template>
              </el-input>
            </el-form-item>
            <el-form-item label="图片高度" v-show="formData.rightImageShow==1">
              <el-input v-model="formData.rightImageHeight" size="mini" type="number" placeholder="高度">
                <template slot="append">px</template>
              </el-input>
            </el-form-item>
            <el-form-item label="跳转链接"  v-show="formData.rightImageShow==1">
            </el-form-item>
            <div style="padding: 0 0 5px 20px"  v-if="formData.rightImageShow==1">
              <app-page-select :clientType="clientType" :page="formData.rightImagePageUrl" @change="formData.rightImagePageUrl=$event"></app-page-select>
            </div>
          </div>
        </el-form>
      </div>
...
export default {
  components: {settingSlot, bgColorSelect, MaterialList,AppPageSelect},
  data() {
    return {
      formDataCopy: {
        background: `#efeff4`,
