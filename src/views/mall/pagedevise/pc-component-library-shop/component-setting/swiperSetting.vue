    ...
    <settingSlot @confirm="confirm" @cancel="cancel" @reset="reset" :showReset="true">
      ...
      <div slot="mainContent">
        <el-form ref="form"  label-width="90px" :model="formData">
          <el-form-item label="轮播样式" v-if="clickComIndex!='indexBanner'">
          ...
          <el-form-item label="图片倒角"  v-if="clickComIndex!='indexBanner'">
          ...
          <el-form-item label="平铺">
            <el-radio-group v-model="formData.fullScreen" >
          ...
          <el-form-item label="背景颜色" v-if="clickComIndex!='indexBanner'">
...
export default {
  ...
  mounted(){
    let that = this;
    if(that.IsEmptyObj(this.showData)){
      that.formData = JSON.parse(JSON.stringify(this.formDataCopy))
    } else {
      that.formData = that.showData
    }
    if(that.clickComIndex=='indexBanner'){
      this.$emit('changeIndexBanner', this.formData)
    }else{
  ...
  methods: {
    ...
    delBtn(index){
      ...
      that.$confirm('是否删除该组件?', '提示', {
        ...
        if(that.clickComIndex=='indexBanner'){
          that.$delete(this.formData.swiperList, index)
          this.$emit('changeCompData', 'swiper', this.formData)
        }else{
    ...
    reset(){
      ...
      if(this.clickComIndex=='indexBanner'){
        this.$emit('changeIndexBanner', this.formData)
      }else{
