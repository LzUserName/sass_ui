...
<template>
  <div class="compSetting">
    <settingSlot @confirm="confirm" @cancel="cancel" @reset="reset" :showReset="true">
      ...
      <div slot="hint">
        <div class="tips-class " >提示：图片宽度自适应100%。</div>
      </div>
        ...
        <el-form ref="form"  label-width="90px" :model="formData">
          <el-form-item label="图片">
            <MaterialList :value="formData.imageUrl?[formData.imageUrl]:[]"  @sureSuccess="formData.imageUrl = $event?$event[0]:''" @deleteMaterial="formData.imageUrl = ''"
                          type="image" :shopId="shopId" :divStyle="'width:100%;margin-bottom:0px;height:90px;line-height: 90px;'" :num="1" ></MaterialList>
          </el-form-item>
          ...
          <div style="padding: 0 0 5px 20px">
            <app-page-select-shop :shopId="shopId" :isRefresh="true" :page="formData.pageUrl" @change="formData.pageUrl=$event"></app-page-select-shop>
          </div>
...
export default {
  components: { settingSlot, MaterialList, AppPageSelectShop  },
  data() {
    return {
      formDataCopy : {
        height: 300,
