    ...
    <basic-container>
      <avue-crud ref="crud"
        ...
        <template slot="picUrlForm"
                  slot-scope="scope">
          <MaterialList v-model="form.picUrl" type="image" :num=1 shopId="-1" :width='form.parentId == "0" ? 360 : 360'
                        :height=80></MaterialList>
          <P>推荐尺寸：720px*160px或同比例尺寸图片</P>
        </template>
        <template slot="picUrl" slot-scope="scope">
          <img style="height: 80px"
               :src="scope.row.picUrl">
        </template>
        <template slot="listActiveBackgroundRelationForm" slot-scope="scope">
          <div>
            ...
            <el-row :gutter="4" v-if="form.suitType == '2'">
              <el-col :span="3" v-for="(item, index) in form.listActiveBackgroundRelation" :key="item.id">
            ...
            <el-cascader v-model="form.listActiveBackgroundRelation" :options="treeGoodsCategory"
            ...
            <el-select v-model="form.listActiveBackgroundRelation" multiple placeholder="请选择店铺" v-if="form.suitType == '4'">
              <el-option
...
<script>
  import {getPage, getObj, addObj, putObj, delObj} from '@/api/mall/activebackground'
  ...
  import {tableOption} from '@/const/crud/mall/activebackground'
  ...
  export default {
    ...
    data() {
      return {
        ...
        selectionActiveBackgroundRelation: []
      }
    },
    watch: {
      'form.suitType'() {
        this.form.listActiveBackgroundRelation = []
    ...
    computed: {
      ...
      permissionList() {
        return {
          addBtn: this.permissions['mall:activebackground:add'] ? true : false,
          delBtn: this.permissions['mall:activebackground:del'] ? true : false,
          editBtn: this.permissions['mall:activebackground:edit'] ? true : false,
          viewBtn: this.permissions['mall:activebackground:get'] ? true : false
        };
      }
    },
    methods: {
      ...
      handleUpdate: function (row, index, done, loading) {
        ...
        let selectionActiveBackgroundRelation = row.listActiveBackgroundRelation
        if (selectionActiveBackgroundRelation && selectionActiveBackgroundRelation.length > 0) {
          let listActiveBackgroundRelation = []
          selectionActiveBackgroundRelation.forEach(function (val, index) {//过滤无用数据
            let relationId
            ...
            listActiveBackgroundRelation.push({
              relationId: relationId
            })
          })
          row.listActiveBackgroundRelation = listActiveBackgroundRelation
        } else {
          row.listActiveBackgroundRelation = null
        }
        ...
        if (type == 'edit' || type == 'view') {//如果是修改、查看操作，加载关联商品
          if (this.form.suitType == '2' || this.form.suitType == '3' || this.form.suitType == '4') {
            getObj(this.form.id).then(response => {
              ...
              let listActiveBackgroundRelation = []
              obj.listActiveBackgroundRelation.forEach(i => {
                if(that.form.suitType == '2'){
                  listActiveBackgroundRelation.push(i.goodsSpu)
                }
                if(that.form.suitType == '3' || that.form.suitType == '4'){
                  listActiveBackgroundRelation.push(i.relationId)
                }
              })
              this.form.listActiveBackgroundRelation = listActiveBackgroundRelation
            }).catch(() => {
      ...
      selectionChange2(list) {
        this.selectionActiveBackgroundRelation = list
      },
      subGoods() {
        let listActiveBackgroundRelation = this.form.listActiveBackgroundRelation
        listActiveBackgroundRelation = [...listActiveBackgroundRelation, ...this.selectionActiveBackgroundRelation]
        //去重
        this.form.listActiveBackgroundRelation = unique(listActiveBackgroundRelation)
        this.dialogVisibleGoods = false
      },
      removeRelation(index) {
        this.form.listActiveBackgroundRelation.splice(index, 1)
      },
      getListShopInfo(){
